...
int main()
{
    cv::Mat output = cv::Mat::zeros(cv::Size(800, 800), CV_8UC3);
    ...
    for(int i; i <= 80 ; i++){
      ...
      cv::line(output, cv::Point(x, 0), cv::Point(x2, 800), cv::Scalar(0, 150, 255), 1,8);
      cv::line(output, cv::Point(0, x), cv::Point(800, x2), cv::Scalar(0, 150, 255), 1,8);
